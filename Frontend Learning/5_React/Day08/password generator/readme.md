...
It render first time normally The callback runs after render.second time if [generatepassword] is given then it render again 


useEffect(()=>{
     generatepassword();
  },[])
only first time render and call the generatepassword() second time it not work because dependency array is empty


useEffect(()=>{
     generatepassword();
  })
It render infinite times
...
<!-- ***************************************************************** -->
 ...
 every time this function is re-render when usestate() call and allocate memory it takes lot of time and space 
 generatepassword:4423
 generatepassword:34443423
 generatepassword:4423244


 -> To save this we use the property of closure ;;
 it takes  -> useCallback(callback,[dependencies])

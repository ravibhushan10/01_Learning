...
form.addEventListener('submit',(event)=>{
    ...
    event.preventDefault(); //it prevent the reload the page
    
   
    
    const name=document.getElementById('fname').value;
    const surname=document.getElementById('lname').value;
    // form always gives you input in the form of string 
    ...
    // // you can randomly select or fix the victimCardCompliments
    // const index=Math.floor(Math.random()*20);
    // const fourth_message=victimCardCompliments[index];

    const ind=(day*month)%20;
    const fourth_message=victimCardCompliments[ind];

    
    
    // Here i'm fix the recommendation for particular user
    // "Rohit Negi" Day 
    //  5  *    4  * 4= 80%30;  
    const index1=(name.length*surname.length*day)%30;

...
function RandomQuestion(){
    ...
    //  use of set takes too much time it increase your time complexity , here we have limited data(no of question) but when we have large data set in backend then fetching data takes too much time so use better approach so your resources will less use
    // it takes almost more than O(n) time  
    // when same element comes then it ignore so increase

    // const index=Math.floor(Math.random()*20);
    // // use set for unique element
    // // To conform we don't get the same index(same question) we use set
...
//  3: Fisher algorithm(o(n)) exact 


// 4: use math.random and replace the value to last value and apply math.random again n-1
...
problem.forEach((obj,index)=>{
    ...
    para.textContent=`${index+1}. ${obj.question}`  //obj.question
    ...
    obj.options.forEach((data)=>{
        ...
        // create textnode(innertext of label):Sachin Tendulkar
